...
from transformer.data.train_val_image_folder_data_module import (
    TrainValImageFolderDataModule,
)
from transformer.models.pretrained_vit_b import PretrainedViTBFactory
from transformer.training.energy.gpu_energy_meter import EMLGPUEnergyMeter
from transformer.training.energy.slurm_gpu_selector import SlurmGPUSelector
from transformer.training.vit_b_trainer import ViTBTrainer
from transformer.training.vit_b_training_config import ViTBTrainingConfig
...
class PretrainedViTBTrainingApplication:
    ...
    def run(self) -> None:
        ...
        gpu_meter = EMLGPUEnergyMeter(
            SlurmGPUSelector(self.config.eml_gpu_index),
        )

        result, measurements, elapsed, device_key = gpu_meter.measure(
            lambda: trainer.fit(self.config.epochs),
        )
        gpu_metrics = gpu_meter.extract_metrics(measurements, device_key)
        ...
        print(f"EML elapsed time: {elapsed:.2f} s")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Physical NVML GPU index used by EML. If omitted, it is inferred "
            "from CUDA_VISIBLE_DEVICES or Slurm variables."
        ),
    )
...
def build_config(args: argparse.Namespace) -> ViTBTrainingConfig:
    ...
    return ViTBTrainingConfig(
        ...
        device=args.device,
        eml_gpu_index=args.eml_gpu_index,
    )
