...
File: quantization/int4/model_factory.py

Description:
    Factory responsible for loading the FP32 VisionTransformer, applying
    INT4 weight-only quantization and computing model statistics.

    INT4 quantization uses symmetric per-tensor quantization on every
    ``nn.Linear`` weight matrix.  Weights are mapped to the signed 4-bit
    range [-8, 7] using a per-tensor scale factor, then immediately
    ...
    The reported size is the theoretical packed size (4 bits per weight
...
class ModelFactory:
    """Load, INT4-quantize and inspect VisionTransformer checkpoints.
    ...
    def quantize_int4(self, model_fp32: nn.Module) -> nn.Module:
        """Return a deep copy of *model_fp32* with INT4-simulated weights.

        Every ``nn.Linear`` weight tensor is quantized to the signed 4-bit
        range [-8, 7] via symmetric per-tensor scaling, then dequantized
        back to FP32.  All other parameters (biases, LayerNorm, embeddings)
        are left unchanged.
        ...
        Returns:
            New FP32 model whose Linear weights carry only 4-bit information.
        """
    ...
    def theoretical_size_mb(self, model: nn.Module) -> float:
        """Compute the theoretical packed size at 4 bits per weight value.

        ``nn.Linear`` weight matrices are counted at 4 bits each; all other
        ...
        for name, param in model.named_parameters():
            if "weight" in name and param.ndim >= 2:
                weight_bits += param.numel() * 4
            else:
                other_bytes += param.numel() * 4  # FP32
    ...
    def _quantize_tensor(self, tensor: torch.Tensor) -> torch.Tensor:
        """Quantize *tensor* to INT4 range then dequantize to FP32.
        ...
        if abs_max == 0:
            return tensor
        scale = abs_max / self._INT4_MAX
        quantized = torch.clamp(
            torch.round(tensor / scale), self._INT4_MIN, self._INT4_MAX
        )
