...
File: quantization/int4/quantization_stats.py

Description:
    Dataclass that stores benchmark results for a single model variant
    (FP32 baseline or INT4 weight-only quantized).
"""
...
class QuantizationStats:
    ...
    Attributes:
        label: Human-readable variant name (e.g. ``"FP32"``, ``"INT4 (W4A32)"``).
        total_params: Total number of model parameters.
        disk_size_mb: Model size in megabytes.  For FP32 this is the actual
            checkpoint size on disk; for INT4 it is the theoretical packed
            size if weights were stored at 4 bits each.
