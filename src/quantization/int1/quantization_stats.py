...
File: quantization/int1/quantization_stats.py

Description:
    Dataclass that stores benchmark results for a single model variant
    (FP32 baseline or INT1 binary weight quantized).
"""
...
class QuantizationStats:
    ...
    Attributes:
        label: Human-readable variant name (e.g. ``"FP32"``, ``"INT1 (Binary)"``).
        total_params: Total number of model parameters.
        disk_size_mb: Model size in megabytes.  For FP32 this is the actual
            checkpoint size on disk; for INT1 it is the theoretical packed
            size if weights were stored as single bits.
