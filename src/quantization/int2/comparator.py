...
File: quantization/int2/comparator.py

Description:
    Orchestrator that benchmarks the FP32 VisionTransformer baseline
    against its INT2 weight-only quantized variant and saves a comparison
    figure.

    INT2 simulated quantization:
        Each ``nn.Linear`` weight tensor is mapped to the signed 2-bit
        range [-2, 1] with a symmetric per-tensor scale, then dequantized
        to FP32.  Inference runs on GPU.  The reported model size is the
        theoretical packed size (2 bits per weight value).
"""
...
class QuantizationComparator:
    """Benchmark FP32 vs INT2 weight quantization and generate a report.
    ...
    def __init__(
        ...
        output_path: str | Path = (
            "outputs/figures/quantization/quantization_int2_comparison.png"
        ),
    ) -> None:
    ...
    def run(self) -> list[QuantizationStats]:
        ...
        Returns:
            A list with two ``QuantizationStats``: FP32 first, INT2 second.
        """
        ...
        print("\n--- Benchmarking INT2 (W2A32) ---")
        model_int2 = self._factory.quantize_int2(model_fp32)
        theo_mb = self._factory.theoretical_size_mb(model_fp32)
        acc_int2, elapsed_int2 = self._benchmark.run(model_int2, self._test_dir)
        all_stats.append(QuantizationStats(
            label=VARIANT_LABEL["int2"],
            total_params=total_params,
            disk_size_mb=theo_mb,
            accuracy=acc_int2,
            elapsed_sec=elapsed_int2,
        ))
        self._print_stats(all_stats[-1])
    ...
    def _print_summary(stats: list[QuantizationStats]) -> None:
        ...
        Args:
            stats: List with FP32 at index 0 and INT2 at index 1.
        """
        fp32, int2 = stats[0], stats[1]
        size_reduction = (1.0 - int2.disk_size_mb / fp32.disk_size_mb) * 100.0
        speedup = fp32.elapsed_sec / max(1e-9, int2.elapsed_sec)
        acc_delta = int2.accuracy - fp32.accuracy
        ...
        print(
            f"INT2  -> Acc: {int2.accuracy:.2f}% | "
            f"Time: {int2.elapsed_sec:.2f}s | "
            f"Size: {int2.disk_size_mb:.2f} MB"
        )
        print("---")
